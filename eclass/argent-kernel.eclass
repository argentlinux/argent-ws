...
DESCRIPTION="Argent, Argent and ArgOS linux kernel functions and phases"
...
if [ "${K_ROGKERNEL_PATCH_UPSTREAM_TARBALL}" = "1" ]; then
	...
	SRC_URI="http://bpr.bluepink.ro/argent/distfiles/${CATEGORY}/linux-${PVR}+${K_ROGKERNEL_SELF_TARBALL_NAME}.tar.${K_TARBALL_EXT}"
else
	SRC_URI="${KERNEL_URI}"
fi
