...
IUSE="arg-kernel dist-kernel modules-compress modules-sign +strip ${MODULES_OPTIONAL_IUSE}"

RDEPEND="
	sys-apps/kmod[tools]
	arg-kernel? ( virtual/arg-kernel:= )
...
linux-mod-r1_pkg_postinst() {
	...
	if has_version virtual/arg-kernel && ! use arg-kernel; then
		ewarn "virtual/arg-kernel is installed, but USE=\"arg-kernel\""
		ewarn "is not enabled for ${CATEGORY}/${PN}."
		ewarn "It's recommended to globally enable the arg-kernel USE flag"
		ewarn "to automatically trigger rebuilds on Argent kernel updates"
	fi
