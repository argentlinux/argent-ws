...
src_prepare() {
	ecm_src_prepare
	# only remove Archive dependency if packagekit is disabled
	...
	# only disable markdown if packagekit is disabled
	if ! use packagekit; then
...
src_configure() {
	local mycmakeargs=(
		-DCMAKE_DISABLE_FIND_PACKAGE_packagekitqt6=$(usex packagekit OFF ON)
